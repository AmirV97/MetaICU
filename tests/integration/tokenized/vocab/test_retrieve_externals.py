...
class RetrieveExternalsTests(unittest.TestCase):
    ...
    def test_write_readme_only_documents_repositories_and_athena_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            parent_readme = (parent_dir / "README.md").read_text()
            self.assertIn("build-amsterdam-vocab paths.parent_dir=", parent_readme)
            self.assertIn("run.mode=rebuild", parent_readme)
            self.assertNotIn("step=build_vocab", parent_readme)
