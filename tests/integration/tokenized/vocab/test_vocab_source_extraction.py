...
class SourceVocabExtractionTests(unittest.TestCase):
    ...
    def test_raw_source_vocab_preserves_latin1_units_and_labels(self) -> None:
        numeric_path = self.raw_data / "numericitems.csv"
        numeric = pd.read_csv(numeric_path)
        numeric.loc[len(numeric)] = {
            "admissionid": 1,
            "itemid": 10008,
            "item": "Temperatuur °C",
            "tag": "",
        ...
        numeric.to_csv(numeric_path, index=False, encoding="latin1")
        ...
        vocab = extract_source_vocab(config).set_index("source_token")

        self.assertIn("LAB//10008//µg/l", vocab.index)
        self.assertEqual(vocab.loc["LAB//10008//µg/l", "source_label"], "Temperatuur °C")
