...
reference (`vocab_pipeline/data/policy_manifests/tier0_baseline_resolution.csv`) rather than
re-implemented; this is the one open scope item, documented in `target_resolution.py`'s
docstring. Everything downstream of it -- curated policy manifests v4-v11, zero-sentinel, lab
consolidation, namespace canonicalization, lab-role assignment, GCS component policy -- runs
fresh every build via `policies/engine.py`, in the fixed order specified below.

The v4-v11 curated clinical-review layers are versioned manifests, generated by diffing the
historical artifact lineage (v1 through v16) rather than re-implementing the original review.
...
- Baseline + manifests + policy code reproduce `v16` with zero row-level differences across all
  9,014 tokens, checked against both the historical artifact lineage and the package code.
- A build against the raw AmsterdamUMCdb CSVs (HPC batch job) reproduces `v16` with zero
  semantic differences on the 8,750 source tokens present in both.
- That same run surfaced a pre-existing bug in `source_vocab.py`: numeric-unit extraction
  treated only null/empty `unit` values as missing, not the raw export's literal text `"None"`,
  which mis-keyed ~65M rows of vitals (e.g. `Saturatie (Monitor)`, `Ademfrequentie Monitor`)
  under a `//None` token instead of `//UNKNOWN`. Fixed in `_is_null_text_expr`.

`v16`'s content is now `mappings/aumc_supplied_vocab.csv` (and the packaged
`src/metaicu/aumcdb/tokenized/data/` copy). The three hardcoded counts in
`tests/test_supplied_vocab_contract.py` are updated to match (4,836->4,837 emitted, 515->516
LAB rows, 14,301,315->14,301,350 LAB row sum).

`run.allow_unresolved_source_tokens` (default `false`) and `validate_supplied_vocab(...,
