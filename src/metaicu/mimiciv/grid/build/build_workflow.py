...
def build_pre_scale_grid(config: GridDatasetConfig) -> PreScaleGrid:
    ...
    assignments = assign_splits(admissions, config.train_frac, config.val_frac, config.test_frac, config.split_seed)
...
def finish_grid_dataset(
    ...
    if config.one_hot:
        grid, grid_encoding_schema = one_hot_encode_categorical(grid, matches_with_derived, start_pos=next_categorical_pos)
