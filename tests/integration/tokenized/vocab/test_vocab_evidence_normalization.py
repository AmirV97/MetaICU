...
from metaicu.aumcdb.tokenized.vocab_pipeline.build_workflow import packaged_supplied_vocab
...
""".strip(),
    ...
    def test_build_vocab_parent_dir_uses_default_subfolders(self) -> None:
        ...
        self.assertFalse((parent_dir / "audits/vocab/vocab_pipeline_source_vocab.csv").exists())

    def test_build_vocab_from_outside_checkout_installs_packaged_supplied_vocab(self) -> None:
    ...
    def test_build_vocab_installs_explicit_supplied_vocab(self) -> None:
        reviewed_vocab = self.root / "reviewed_vocab.csv"
        reviewed_vocab.write_bytes(packaged_supplied_vocab().read_bytes())
        output_vocab = self.root / "explicit_vocab/aumc_supplied_vocab.csv"

        cmd = [
            sys.executable,
            str(PIPELINE_ROOT / "scripts/build_amsterdam_vocab.py"),
            f"paths.supplied_vocab={reviewed_vocab}",
            f"paths.output_vocab={output_vocab}",
        ]
        subprocess.run(cmd, cwd=self.root, check=True)

        self.assertEqual(output_vocab.read_bytes(), reviewed_vocab.read_bytes())
