...
missing class represents, not a value to fabricate). Ported verbatim from
AUMC_grid_pipeline/grid/encode.py (dataset-agnostic).
...
train/val/test split. All 5 of M4's categorical tags (mgcs/vgcs/egcs/rass/airway) declare a
standardized_label per category as of 2026-07-29 (see mimic_grid_feature_manifest_review.md).
...
def one_hot_encode_columns(df, vocab, start_pos=0):
    """Generic one-hot engine, vocab-agnostic -- works on ANY DataFrame + {tag: [categories]}
    vocab dict, not just manifest-derived categorical features. Ported from MetaICU's
    grid/build/encode.py (dataset_EDA's AUMC_grid_pipeline never split this out -- its
    one_hot_encode_categorical had the same logic inlined, manifest-vocab-only; MetaICU
    generalized it so static features like sex/adm could reuse it too, see
    grid.extract_static.STATIC_CATEGORICAL_VOCAB).

    df: DataFrame carrying every tag in vocab as a single string-label column (real nulls =
    missing). vocab: {tag: sorted_list_of_categories}. start_pos: position_global to start
    numbering from (lets a caller concatenate this call's encoding_schema after an earlier one,
    e.g. static features encoded before grid features).

    Returns (df, encoding_schema, next_pos): df has each vocab tag column REPLACED by its
    one-hot expansion; encoding_schema is a list of row-dicts (feature, category, column_name,
    position_in_feature, position_global); next_pos is the position_global to continue from for
    a subsequent call."""
    encoding_schema = []
    ...
    for tag, categories in vocab.items():
        ...
        if len(set(col_names)) != len(col_names):
            raise ValueError(
                ...
                f"categories produced the same one-hot column name, would silently overwrite "
                f"each other. Fix the vocab or _sanitize() before proceeding."
            )
        new_cols = []
...
def save_categorical_encoding(encoding_schema, output_path):
    ...
    (feature, category, column_name, position_in_feature, position_global) -- the record of
    which physical column each (feature, category) pair maps to and where it sits in the
    one-hot vector, both within its own feature's block and across all categorical features'
    columns concatenated in encoding order."""
