"""
Copied verbatim from AUMC_grid_pipeline/grid/plausibility_bounds.py -- these are physiology
bounds keyed by feature tag (same 129-feature definitions AUMC and MIMIC share), not AUMC data
quirks, so they apply unchanged here. NOT yet re-validated against MIMIC's own value
distributions the way AUMC's were (see this file's own docstring below on how AUMC's bounds
were calibrated) -- a real follow-up once MIMIC extraction output can be inspected; kept as a
reasonable, already-literature-checked starting point rather than no bound at all.
